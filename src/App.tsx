...
function App() {
  ...
  useEffect(() => {
    console.log("Orderbook state changed - Buy orders:", buyOrders.length, "Sell orders:", sellOrders.length);
  ...
  useEffect(() => {
    ...
    const callback = (orderbooks: any) => {
      ...
      try {
        ...
        } else if (orderbooks && Array.isArray(orderbooks) && orderbooks.length > 0 && orderbooks[0]?.orderbook) {
        ...
        if (orderbook && (orderbook.buys || orderbook.sells)) {
          ...
          console.log("No valid orderbook data found, but setting loading to false");
          setOrderbookLoading(false);
        }
      } catch (err) {
  ...
  const getCurrentPairData = () => {
    return tradingPairs.find(pair => pair.ticker === selectedPair) || tradingPairs[0];
  };
  ...
  const fetchUserBalances = async (injectiveAddress: string) => {
    try {
      ...
      const formattedBalances: TokenBalance[] = balancesArray.map((balance: any) => {
      ...
      setUserPanelError(`Failed to fetch balances: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
  ...
  const fetchUserPositions = async (injectiveAddress: string) => {
    try {
      ...
      setUserPanelError(`Failed to fetch positions: ${err instanceof Error ? err.message : 'Unknown error'}`);
      setPositions([]);
    } finally {
    ...
    try {
      ...
      if (orderType === 'limit') {
        ...
        console.log("Price conversion:", {
          ...
          formula: `${parseFloat(price)} × 10^(${marketWithMultipliers.quoteDecimals} - ${marketWithMultipliers.baseDecimals}) = ${parseFloat(price)} × 10^${marketWithMultipliers.quoteDecimals - marketWithMultipliers.baseDecimals}`
        });
        ...
        console.log("Market order price conversion:", {
          ...
          chainPrice: chainPrice,
          formula: `${marketPrice} × 10^(${marketWithMultipliers.quoteDecimals} - ${marketWithMultipliers.baseDecimals}) = ${marketPrice} × 10^${marketWithMultipliers.quoteDecimals - marketWithMultipliers.baseDecimals}`
        });
      ...
      try {
        ...
        // Access the base_account structure correctly
        ...
        // For wallet-based transactions, we'll create a simpler broadcast approach
        // Since we don't have the private key (wallet manages it), we'll simulate the transaction
        // In a real implementation, you'd use a wallet connector library like @injectivelabs/wallet-ts
        
        // For now, show detailed information about what would be broadcasted
        const simulatedTxHash = `0x${Math.random().toString(16).substr(2, 64)}`;
        
        console.log("Transaction would be broadcasted with:", {
          message: msg,
          signBytes: signBytesHex,
          signature: signature,
          txRaw: txRaw
        });
        ...
        setOrderSuccess(`✅ Order transaction prepared successfully!
                        Simulated TX Hash: ${simulatedTxHash}
                        Market: ${currentMarket.ticker}
                        Type: ${orderType.toUpperCase()} ${orderSide.toUpperCase()}
                        Quantity: ${quantity}
                        ${orderType === 'limit' ? `Price: ${price}` : 'Market Order'}
                        
                        Note: This is a demo. In production, use @injectivelabs/wallet-ts for real broadcasting.`);
        
        // Clear form
        setPrice('');
        setQuantity('');
        
        // Refresh user data to show updated balances/positions
        setTimeout(() => {
          refreshUserData();
        }, 2000);
        ...
        setOrderError(`Failed to broadcast transaction: ${broadcastError instanceof Error ? broadcastError.message : 'Unknown error'}`);
      }
                ...
                <div className="market-info-display">
                  <div className="token-info">
                    ...
                    <span className="token-symbol">
                      {getCurrentPairData()?.baseToken?.symbol || getCurrentPairData()?.baseDenom || 'N/A'}
                    </span>
                    ...
                    <span className="token-symbol">
                      {getCurrentPairData()?.quoteToken?.symbol || getCurrentPairData()?.quoteDenom || 'N/A'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="trading-interface">
              ...
              {orderbookLoading ? (
                ...
                <>
                  <div className="debug-info" style={{ fontSize: '12px', color: '#666', marginBottom: '10px' }}>
                    Buy Orders: {buyOrders.length} | Sell Orders: {sellOrders.length} | Last Update: {new Date().toLocaleTimeString()}
                  </div>
                  ...
                  <div className="sell-orders">
                    ...
                    {sellOrders.slice().reverse().map((order, index) => (
                      <div key={`sell-${index}`} className="order-row sell-order" onClick={() => handlePriceClick(order.price)}>
                        <span className="price">{parseFloat(order.price).toFixed(6)}</span>
                        <span className="quantity">{parseFloat(order.quantity).toFixed(2)}</span>
                      </div>
                    ))}
                    {sellOrders.length === 0 && (
                  ...
                  <div className="buy-orders">
                    {buyOrders.map((order, index) => (
                      <div key={`buy-${index}`} className="order-row buy-order" onClick={() => handlePriceClick(order.price)}>
                        <span className="price">{parseFloat(order.price).toFixed(6)}</span>
                        <span className="quantity">{parseFloat(order.quantity).toFixed(2)}</span>
                      </div>
                    ))}
          ...
          <div className="trading-form-column">
            <div className="trading-form">
              ...
              <button 
                ...
                disabled={isPlacingOrder || !quantity || (orderType === 'limit' && !price) || !isConnected}
              >
                {isPlacingOrder ? (
                  <>
          ...
          <div className="user-panel-column">
            <div className="user-panel">
              <div className="user-panel-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                <h3>USER PANEL</h3>
                {isConnected && (
                  <button 
              ...
              {!isConnected && (
                <div className="connect-prompt" style={{ textAlign: 'center', padding: '20px', color: '#666' }}>
              ...
              {isConnected && (
                <>
                  ...
                  <div className="balances-section">
                    <h4>BALANCES {balancesLoading && <span style={{ fontSize: '12px', color: '#666' }}>(Loading...)</span>}</h4>
                  ...
                  <div className="active-orders-section">
                    <h4>ACTIVE POSITIONS {positionsLoading && <span style={{ fontSize: '12px', color: '#666' }}>(Loading...)</span>}</h4>
